...
class Metrics(plugin: JavaPlugin, serviceId: Int) {
    ...
    init {
        this.plugin = plugin
        // Get the config file
        val bStatsFolder = File(plugin.dataFolder.getParentFile(), "bStats")
    ...
    class MetricsBase(
         ...
         */
        init {
            val scheduler = ScheduledThreadPoolExecutor(1) { task: Runnable? -> Thread(task, "bStats-Metrics") }
     ...
     * @param chartId The id of the chart.
     * @param callable The callable which is used to request the chart data.
     */(chartId: String?, private val callable: Callable<Map<String, IntArray>>) : CustomChart(chartId) {
        @get:Throws(Exception::class)
        override val chartData: JsonObjectBuilder.JsonObject?
            protected get() {
                val valuesBuilder = JsonObjectBuilder()
     ...
     * @param chartId The id of the chart.
     * @param callable The callable which is used to request the chart data.
     */(chartId: String?, private val callable: Callable<Map<String, Map<String, Int>>>) : CustomChart(chartId) {
        @get:Throws(Exception::class)
        override val chartData: JsonObjectBuilder.JsonObject?
            get() {
                val valuesBuilder = JsonObjectBuilder()
    ...
    class JsonObjectBuilder {
        ...
        fun appendField(key: String?, values: IntArray?): JsonObjectBuilder {
            requireNotNull(values) { "JSON values must not be null" }
            val escapedValues = Arrays.stream(values).mapToObj { i: Int -> java.lang.String.valueOf(i) }
                .collect(Collectors.joining(","))
            appendFieldUnescaped(key, "[$escapedValues]")
            return this
        }
